...
## plot
if(ix == 1) {
png(paste0(out_02,"/02 besag model graph mat_", ix,".png"), width = 1000, height = 800, res = 150)
image(inla.graph2matrix(grid_graph),xlab="",ylab="")
dev.off()

png(paste0(out_02,"/02 besag model graph fig_", ix,".png"), width = 1000, height = 800, res = 150)
plot(grid_graph)
dev.off()
}
# model ------------------------------------------------------------------------
