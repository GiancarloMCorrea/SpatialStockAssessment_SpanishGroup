...
type_model = '1area_25_lenSelex'
main_dir = paste0(getwd(),"/",type_model)
...
# Output dir:
outdir = paste0(type_model, '_output')
dir.create(outdir)
...
for(j in seq_along(mydata)) { 
  ...
  png(file=paste(dir,"/a_SSplot.png",sep=""), width = 1200, height = 800,
      pointsize=18)
  par(mfrow=c(3,3))
  SSplotCatch(output, subplots=10)
  SSplotSummaryF(output)
  SSplotBiology(output,subplots = 1)
  SSplotTimeseries(output, subplot = 14, minyr = min(years))
  SSplotRecdevs(output, subplots=1)
  SSplotBiology(output,subplots = 6)
  SSplotSelex(output, subplot = 1)
  SSplotTimeseries(output, subplot = 7)
  SSplotTimeseries(output, subplot = 7, minyr = min(years))
  dev.off()
...
for (i in 1:length(gdat)){
  f_dat[,i]=gdat[[i]]$F_val # FIND ERROR HERE
}
...
for (i in 1:length(gdat)){
  r_dat[,i]=gdat[[i]]$rec_value # FIND ERROR HERE
}
